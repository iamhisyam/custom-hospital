...
class PatientLabResultController {
    private $logger;
    private $db;
    private $validator;
    ...
    private $routeName = "patient_labs_result";
    ...
    public function all(Request $request, Response $response) {
        ...
        $patient_lab_result = LabSetup::skip($range[0])->take($take)->orderBy($sort[0],$sort[1])->get();
        return $response
            ->withHeader('Content-Range',"$this->routeName ".$range[0]."-".$range[1]."/".$dataCount)
            ->withJson(['data' => $patient_lab_result], 200);
    }
    
    // GET /branchs/{id}
    // Retrieve patient_lab_result data by ID
    public function find(Request $request, Response $response, $args) {
        $this->logger->addInfo("GET /$this->routeName/".$args['id']);
        $user = $request->getAttribute('user');
        $patient_lab_result = LabSetup::find($args['id']);
        if ($patient_lab_result) {
            return $response->withJson([
                //'success' => true,
                'data' => $patient_lab_result
            ], 200);
        } else {
            return $response->withJson([
                'success' => false,
    ...
    public function create(Request $request, Response $response) {
        ...
        if ($validator->isValid()) {
            ...
            $patient_lab_result =  LabSetup::firstOrCreate($newData);
         
            $this->logger->addInfo("POST valid /$this->routeName ".  $patient_lab_result );
            
            return $response->withJson([
                'success' => true,
                'id' => $patient_lab_result->id
            ], 200);
        } else {
            // Validation error
    ...
    public function update(Request $request, Response $response, $args) {
        ...
        // check patient_lab_result ID exists
        $patient_lab_result = LabSetup::find($args['id']);
        if (!$errors && !$patient_lab_result) {
    ...
    // DELETE /branchs/{id}
    // Delete a patient_lab_result
    public function delete(Request $request, Response $response, $args) {
        $data = $request->getParsedBody();
        $user = $request->getAttribute('user');
        $errors = [];
        // check category ID exists
        $patient_lab_result = LabSetup::withTrashed()->find($args['id']);
        if (!$errors && !$patient_lab_result) {
            $errors = ['LabSetup not found: '.$args['id']];
        }
        if (!$errors) {
            $deleted = (isset($data['force']) && !empty($data['force'])) ? $patient_lab_result->forceDelete() : $patient_lab_result->delete();
